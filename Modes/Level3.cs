...
{
    public class Level3 : AbstractGameDesign
    {
        private List<Block> blocks = new List<Block>();
        private List<Block> buttons = new List<Block>();
        private List<Block> elements = new List<Block>();

        private Block exit_button, hint_button, start_button, static_element, var_block1, var_block2, var_block3, var_block4, loop_block, if_block;
        ...
        private string task = "1111"; //here is the task result of this level 
        ...
        public override void LoadContent(ContentManager Content)
        {
            ...
            var_block3 = new Block(Content.Load<Texture2D>($"zmienna2"), Data.BlockType.ZMIENNA);
            var_block3.setRectangle(new Rectangle(3, 230, var_block3.texture.Width, var_block3.texture.Height));
            var_block4 = new Block(Content.Load<Texture2D>($"zmienna2"), Data.BlockType.ZMIENNA);
            var_block4.setRectangle(new Rectangle(3, 330, var_block3.texture.Width, var_block3.texture.Height));
            loop_block = new Block(Content.Load<Texture2D>($"petla2"), Data.BlockType.PETLA);
            loop_block.setRectangle(new Rectangle(3, 430, loop_block.texture.Width, loop_block.texture.Height));
            if_block = new Block(Content.Load<Texture2D>($"warunek2"), Data.BlockType.WARUNEK);
            if_block.setRectangle(new Rectangle(3, 530, if_block.texture.Width, if_block.texture.Height));

            blocks.Add(var_block1);
            blocks.Add(var_block2);
            blocks.Add(var_block3);
            blocks.Add(var_block4);
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if(ms_current.LeftButton == ButtonState.Pressed && ms_rect.Intersects(hint_button.rect) && ms_old.LeftButton == ButtonState.Released){ //Hint
                Data.displayText1 = Data.lvl3Text1;
                Data.displayText2 = Data.lvl3Text2;
